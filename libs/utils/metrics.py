...
class ANETdetection(object):
    ...
    def wrapper_compute_average_precision(self, preds):
        ...
        confusion_matrix = {cls: {'tp': 0, 'fp': 0} for cls in self.activity_index.keys()}
        ...
        for i,  cidx in enumerate(self.activity_index.values()):
            ap[:, cidx] = results[i][0]  # Update AP for the current class

            # Update confusion matrix for the current class

            class_confusion_matrix = results[i][1]
            if class_confusion_matrix == 0.0:
                continue


            for cls, counts in class_confusion_matrix.items():
                if cls not in confusion_matrix:
                    confusion_matrix[cls] = {'tp': 0, 'fp': 0}
                confusion_matrix[cls]['tp'] += counts['tp']
                confusion_matrix[cls]['fp'] += counts['fp']
        maP_per_class = ap.mean(axis=0)

        return ap, confusion_matrix, maP_per_class
    ...
    def evaluate(self, preds, verbose=True):
        ...
        # print('Here I am printing the activity index: ', self.activity_index)
        # print('Here I am printing the activity level: ', preds['label'])
        # compute mAP
        self.ap, self.confusion_matrix, self.maP_per_class = self.wrapper_compute_average_precision(preds)
        ...
        if verbose:
            ...
            # print('map per class: ', self.maP_per_class.mean())
...
def compute_average_precision_detection(
    ...
    classes = ground_truth['label'].unique()

    confusion_matrix = {cls: {'tp': 0, 'fp': 0} for cls in classes}

    # Iterate over predictions
    for idx, this_pred in prediction.iterrows():
        pred_class = this_pred['label']

        # Ensure the class is in the confusion matrix

        # Iterate over tiou_thresholds and update confusion matrix
        for tidx in range(len(tiou_thresholds)):
            if tp[tidx, idx] == 1:
                confusion_matrix[pred_class]['tp'] += 1
            elif fp[tidx, idx] == 1:
                confusion_matrix[pred_class]['fp'] += 1

    return ap, confusion_matrix
